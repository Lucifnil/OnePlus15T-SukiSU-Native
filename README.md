...
5.29 W、机身峰值 32.2 °C、电池峰值 30.4 °C，未触发安全停止；r33 生产版随后
完成普通零号大坝真实对局回归。约 5 分钟有效负载段内 CPU 持续约 48–55 °C，
一次场景重新挂载瞬时达到 69.0 °C 后立即回落，机身峰值约 39.0 °C、电池峰值
约 36.7 °C；Hmbird 与 `sched_ext` 始终按场景正常挂载/卸载，没有复现 r30 的
持续 90 °C、持续约 6 W 或调度崩溃。r33 因此作为修复风驰 split-BTF 兼容问题
并完成真实对局验证的正式 Release。
